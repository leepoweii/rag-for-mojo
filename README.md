...
雖然使用的是小模型（GPT-4o-mini），單次調用速度不慢，但多次調用會產生累積的成本負擔。對於高頻使用場景，這可能成為營運上的限制。

**未來研究方向（Future Work）**:

本專案識別出以下可能的改進方向，供未來研究探索：

1. **Code-Assisted 架構設計**
   設計混合架構，讓程式邏輯處理 routine 任務，LLM 只在關鍵決策點介入。例如：使用規則引擎或輕量級 NLP 工具進行簡單的查詢拆解，只在複雜語意判斷時才調用 LLM。

2. **批次處理優化**
   探索將多個步驟合併為單次 LLM 調用的可行性，例如在一次 prompt 中同時完成「查詢拆解」和「檢索策略規劃」，減少 API 調用次數。

3. **本地小模型替代**
   針對特定任務（如查詢拆解），研究是否可以使用輕量級開源模型（如 Llama 3.2 3B）在本地運行，降低 API 成本。

4. **智能路由機制**
   根據問題複雜度動態決定是否需要調用 LLM。簡單問題（如單一子句）可直接進入檢索階段，跳過查詢拆解步驟。

**現狀說明**: 本專案為研究性質，優先驗證 RAG 技術可行性。上述成本優化方向需根據實際使用量和預算進行權衡取捨。
